...
the status text, that cjdns is green with the word `[ACTIVE]`. If you see this text, you have succesfully connected to the remote peer,
...
command to terminal and you should see green text where Yggdraisl is printed with the word `[ACTIVE]` present.
You are now connected to the remote peer with Yggdrasil.if you see`[INACTIVE]`, then you need to check your code
for typos. Make sure there are` ""` surrounding string.  
